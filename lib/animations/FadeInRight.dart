...
class FadeInRight extends StatefulWidget {
  ...
  const FadeInRight({Key? key, required this.child, this.startDuration, this.animationDuration, this.size, this.parentObject}) : super(key: key);
...
class _FadeInRightState extends State<FadeInRight> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    Library.debugPrint(animationID + " " + "FadeInRight");
    ...
    _animation = Tween<double>(begin: 1, end: 1).animate(CurvedAnimation(curve: Curves.easeInOut, parent: _controller));
    ...
    Timer(startDuration!, () {
      ...
      Library.debugPrint("FadeInRight status : " + _controller.status.toString());
    });
  }
        ...
        child: Opacity(
          ...
          child: Opacity(
            ...
            child: Transform.translate(
              offset: Offset(size - (size * _controller.value), 0),
              child: child,
            ),
          ),
        ),
